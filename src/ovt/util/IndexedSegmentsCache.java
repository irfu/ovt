 ...
 * slot (indexed object in IndexObjectsCache) covers a specific interval of t
 * values. For the purposes of OVT, "t" likely represents time, but it can in
 * principle represent anything. The user of the class has to implement methods
 * for splitting up data in cache units and putting data together from cache
 * units.
 *
 * @author Erik P G Johansson, erik.johansson@irfu.se
 */
// PROPOSAL: Convert to using "long" for indexing objects instead of "int".
public class IndexedSegmentsCache {

    private static final double T_REQUEST_MARGIN = 0.1;   // TEMP. Change into general variable? Remove?
    ...
    public interface DataSource {
        ...
        /**
         * Get index for the cache slot that covers/contains this t value.
         */
        public int getCacheSlotIndex(double t);


        /**
         * Produce cache units from data.
         */
        public List<CacheSegment> getCacheSegments(
                int i_beginInclusive,
                int i_endExclusive,
                Object getCacheSegmentsArgument) throws IOException;
         ...
         * data.
         */
        public Object extractDataFromCacheSegments(
                List<CacheSegment> cacheSegments,
                int i_beginUnitArrayInclusive,
                int i_endUnitArrayExclusive);
    ...
    public IndexedSegmentsCache(
            ...
            double tMinInclusive, double tMaxInclusive,
            int proactiveFillMargin) {
        this.dataSource = mDataSource;
        this.i_unitMinInclusive = dataSource.getCacheSlotIndex(tMinInclusive);
        this.i_unitMaxInclusive = dataSource.getCacheSlotIndex(tMaxInclusive);
    ...
    public Object getData(
            double beginInclusiveT, double endInclusiveT,
        ...
        Log.log(this.getClass().getSimpleName() + ".getData(beginInclusiveT=" + beginInclusiveT + ", endInclusiveT=" + endInclusiveT
...

        {
            /*=================================================================
             ...
             =================================================================*/
            int unitBeginInclusive = dataSource.getCacheSlotIndex(beginInclusiveT - T_REQUEST_MARGIN);
            int unitEndExclusive = dataSource.getCacheSlotIndex(endInclusiveT + T_REQUEST_MARGIN) + 1;

            // Request units once to cache as many units in sequence as possible.
            // NOTE: The return value is NOT used. The call is used to trigger.
            this.objectsCache.getIndexedObjects(
                    unitBeginInclusive,
        ...
        /*=============================================================
         ...
         beginEndIndexMargin<<(number of data points per unit).
         =============================================================*/
        ...
        /*=====================================================================
         ...
         =====================================================================*/
        final List<CacheSegment> requestedUnits = this.objectsCache.getIndexedObjects(
                i_unitBeginInclusive,
                i_unitEndInclusive + 1, // Argument should be exclusive upper bound.
                acceptCachedUnitFunction,
                newUnitsArgument);

        return dataSource.extractDataFromCacheSegments(requestedUnits, i_beginDataPointInclusive, i_endDataPointExclusive);
    }
    ...
    private class InternalCacheDataSource implements IndexedObjectsCache.DataSource {
        ...
        @Override
        public List<CacheSegment> getIndexedObjects(int i_beginInclusive, int i_endExclusive, Object newUnitsArgument) throws IOException {
            return IndexedSegmentsCache.this.dataSource.getCacheSegments(i_beginInclusive, i_endExclusive, newUnitsArgument);
        }
    }
    ...
    private int[] getCachePosition(double t, int indexSteps, RoundingMode dataIndexRoundingMode,
        ...
        int i_unit = dataSource.getCacheSlotIndex(t);
        List<CacheSegment> requestedUnits;

        requestedUnits = this.objectsCache.getIndexedObjects(
                i_unit,
                i_unit + 1,
                acceptCachedUnitFunction,
                newUnitsArgument);
